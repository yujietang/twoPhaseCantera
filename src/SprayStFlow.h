...
#ifndef CT_SPRAYSTFLOW_H
#define CT_SPRAYSTFLOW_H
...
public:
    ...
    virtual std::string flowType() {
        ...
        //Free Flame propagating in droplet mists
    }
    ...
    const vector_fp& heatConductivity() const{
        return m_tcon;
    }
    ...
    const vector_fp& kappa() const{
        return m_tcon;
    }

    const size_t nsp() const{
        return m_nsp;
    }

    const doublereal zfixed() const{
        return m_zfixed;
    }
    ...
    void setupCloud(const Lagrangian& field)
    {
        cloud = &field;
    }
...
protected:
    ...
    vector_fp m_rho;
    vector_fp m_wtm;
    // // Sh number and Sc number of gas phase mixture
    // vector_fp m_Sh;
    // vector_fp m_Sc;
